...
- [ ] 收紧 answer synthesis 规则，确保结论只基于召回证据
- [ ] 明确区分：
  - 已有证据支持的判断
  - 可能原因
  - 不确定项
- [ ] citation 增加更稳定的 excerpt 生成逻辑
- [ ] 当召回结果质量不足时，返回更明确的“不确定”响应
- [ ] 增加 response schema 测试，覆盖无结果、弱结果、多引用场景
- [ ] 检查并统一 API response 字段命名风格
...
- [ ] 扩充 JSONL 评测集，覆盖至少 10 到 20 条典型 incident
- [ ] 每条样本包含：
  - 输入
  - 期望命中文档
  - 期望关键词或异常类型
